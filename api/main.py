...
REQUEST_COUNT = Counter("heart_api_requests_total", "Total API requests", ["endpoint", "method", "status"])
PREDICTION_COUNT = Counter("heart_api_predictions_total", "Total predictions", ["prediction"])
LATENCY = Histogram("heart_api_request_latency_seconds", "Request latency", ["endpoint"])
...
def predict(record: PatientRecord) -> Dict[str, Any]:
    ...
    logger.info("prediction=%s confidence=%.4f input=%s", pred, confidence, record.model_dump())
    return {"prediction": pred, "risk_label": "heart_disease" if pred == 1 else "no_heart_disease", "confidence": confidence}
